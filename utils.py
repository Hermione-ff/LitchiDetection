...
def infer_uploaded_image(model,model_type):
    """
    执行上传图片的推断
    ...
    try:
        slice_size = int(model_type.split('-')[2])
    except:
        slice_size = 0
        ...
        if st.button("Execution"):
            ...
            with st.spinner("Running..."):
                ...
                if slice_size == 0:
                    ...
                    big_image = concatenate_images(image_list=image_list, image_size=(slice_size, slice_size), num_cols=3, num_rows=3,
                                                   spacing=20)
                    #预测结果大图
                    res_plotted = result.export_visuals()['image']
                    res_times = result.export_visuals()['elapsed_time']
