...
def train_yolov8():
    ...
    name = r'new_train1' #用于存储训练日志和输出结果
    cos_lr = 'True'#使用余弦退火学习率调度策略
    save_dir = r'/content/drive/MyDrive/LitchiDetection/yolov8_recode/new_train1'
    # 构建训练命令
    train_cmd = f'yolo task=detect mode=train model={model_cfg} mode=train data={data_cfg} imgsz={imgsz} ' \
                f'epochs={epochs} batch={batch_size} resume=True device={device} cos_lr={cos_lr} ' \
